...
void loop() {
  ...
  // Leitura dos botões com debounce (somente se não estiver aguardando um objeto sair)
  if (!aguardandoSaida && (currentTime - lastButtonPressTime > debounceDelay)) {
  ...
  if (ldrValor > LIMIAR_LDR) { // Objeto detectado
    if (!objetoDetectado) { // Só conta uma vez por objeto
      quantidade++;
      showData();
      sendData(cor);
      objetoDetectado = true;
      aguardandoSaida = true; // Agora esperamos o objeto sair para ler outro
      delay(200);
    }
  } else { 
    // Objeto saiu, podemos detectar o próximo
